...
function Figura({ src, alt, caption, objectPos = "object-top", aspect = "aspect-[4/3]" }: { src: string; alt: string; caption: string; objectPos?: string; aspect?: string }) {
  return (
    <figure className="overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--shadow-soft)]">
      <SafeImg src={src} alt={alt} loading="lazy" className={`${aspect} w-full object-cover ${objectPos}`} />
